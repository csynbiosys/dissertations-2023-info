...
data.to_csv('newsCrunchbase_links.csv', encoding='utf-8', index=False)
...
for turn in range(1, number_of_links // 10 + 1):
    try:
        ...
        data = pd.DataFrame(data, columns=['links'])
        data.to_csv('newsCrunchbase_links.csv', encoding='utf-8', index=False, mode='a', header=False)
...
extract_article_content.extract_newsCrunchbase('newsCrunchbase_links.csv', 'newsCrunchbase_articles.csv')